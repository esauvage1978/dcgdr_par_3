...
class Step26_ActionFixtures extends Fixture implements FixtureGroupInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        for ($i = 0; $i < \count($data); ++$i) {
            $this->initialise(new Action(), $data[$i], $manager);
        }
    }
        ...
        if (null == $instance) {
            ...
            switch ($data['id_etat']) {
                ...
                case "4":
                    ...
                    $this->workflowManager->applyTransition($instance, WorkflowData::TRANSITION_TO_DEPLOYED, $content, true);
                    break;
                case "5":
                    $this->workflowManager->applyTransition($instance, WorkflowData::TRANSITION_TO_COTECH, '', true);
                    $this->workflowManager->applyTransition($instance, WorkflowData::TRANSITION_TO_CODIR, '', true);
                    $this->workflowManager->applyTransition($instance, WorkflowData::TRANSITION_TO_FINALISED, '', true);
                    $this->workflowManager->applyTransition($instance, WorkflowData::TRANSITION_TO_DEPLOYED, '', true);
